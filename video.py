...
def parse_args(argv=None):
    ...
    parser.add_argument('--video', default='1.mp4', type=str,
                        help='A path to a video to evaluate on. Passing in a number will use that index webcam.')
    parser.add_argument('--video_multiframe', default=2, type=int,
...
def evalvideo(net: Yolact, path: str, out_path: str = None):
    ...
    x[0][i] if x[1][i] is None else x[0][i].to(x[1][i]['detection']['box'].device), [x[1][i]])
